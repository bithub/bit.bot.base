...
from bit.bot.common.interfaces import IFlatten, IBotAgent, \
    ISubscriptions, IAgents
...
def ping_handler(evt):
    ...
    if 'ping' in subs.subscriptions:
        for subscriber in subs.subscriptions['ping']:
            subs.subscriptions['ping'][subscriber](
                json.dumps(dict(emit={'ping': ''})))
...
class Agents(object):
    ...
    def add(self, service, name, agent, freq):
        if service not in self._agents:
            self._agents[service] = []
        self._agents[service].append(name)
        getUtility(IServices).add(
            service, {name: TimerService(freq, agent().scope)})
